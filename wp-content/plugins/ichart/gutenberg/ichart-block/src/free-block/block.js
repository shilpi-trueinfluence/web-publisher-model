	...
	edit: function( props ) {
		const { attributes: { shortcode }, setAttributes } = props;
 		...
 		function showShortcodeModal(e){
			 $('#ichart_shortcode_generator_free_meta_block').prop('disabled', true);
			 $(e.target).addClass('currently_editing');
			jQuery('#ichart-qcld-chart-field-modal').show();
		}

		function insertShortCode(e){
			const shortcode = $('#ichart_shortcode_container').val();
			setAttributes( { shortcode: shortcode } );
			//$('#wpwrap').find('#sm-modal').remove();
			console.log({ shortcode });
		}

		$(document).on('click','.ichart_copy_close', function(e){
			e.preventDefault();
			jQuery('.currently_editing').next('#insert_shortcode').trigger('click');
			$(document).find( '.ichart-chart-field-modal-close').trigger('click');
			jQuery('#ichart-qcld-chart-field-modal').hide();
		});

		$(document).on( 'click', '.ichart-chart-field-modal-close', function(){
			$('.currently_editing').removeClass('currently_editing');
			$('#ichart_shortcode_generator_free_meta_block').prop('disabled', false);
			jQuery('#ichart-qcld-chart-field-modal').hide();
		});
