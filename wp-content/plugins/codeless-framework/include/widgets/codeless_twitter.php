...
class CodelessTwitter extends WP_Widget{
    ...
    function __construct(){
		...
		parent::__construct( 'widget_twitter', 'Specular Twitter Widget', $options );
    ...
    function widget($atts, $instance){
		...
		echo codeless_complex_esc($entries);
    ...
    function update($new_instance, $old_instance) {
		...
		delete_transient('specular_tweetcache_id_'.$instance['username'].'_'.$this->id_base."-".$this->number);
...
$filtered_message = "";
        ...
        $cache = get_transient('specular_tweetcache_id_'.$username.'_'.$widget_id);
        
        if($cache)
        {
          // $tweets = get_option('specular_tweetcache_'.$username.'_'.$widget_id);
        }
       else
       {
           ...
           if (!is_wp_error($response)) 
            {
                        ...
                        set_transient('specular_tweetcache_id_'.$username.'_'.$widget_id, 'true', 60*30);
                        update_option('specular_tweetcache_'.$username.'_'.$widget_id, $tweets);
