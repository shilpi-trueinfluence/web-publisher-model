...
function like_scripts() {
	wp_enqueue_script( 'jm_like_post', cl_asset_url( 'js/post-like.js' ), array( 'jquery' ) );
	...
	if ( isset( $_POST['jm_post_like'] ) ) {
		...
		if ( is_user_logged_in() ) { // user is logged in
			...
			if ( !AlreadyLiked( $post_id ) ) { // like the post
				...
				echo codeless_complex_esc($post_like_count); // update count on front end
			...
			$ip = codeless_server_var('REMOTE_ADDR'); // user IP address
...
function AlreadyLiked( $post_id ) { // test if user liked before
	if ( is_user_logged_in() ) { // user is logged in
		...
		$ip = codeless_server_var("REMOTE_ADDR"); // Retrieve current user IP
 ...
 * (5) Front end button
 */
function codeless_framework_get_post_like_link( $post_id ) {
	...
	$output = '<a href="#" class="jm-post-like'.esc_attr( $class ).'" data-post_id="'.esc_attr( $post_id ).'">'.$heart.'&nbsp;'.$count.'</a><span class="jm-load"></span>';
	return $output;
}
    ...
    <table class="form-table">
        <tr>
			...
			if ( $count > 0 ) {
				...
				echo codeless_complex_esc($like_list);
			} else {
...
function jm_like_shortcode() {
	return codeless_framework_get_post_like_link( get_the_ID() );
}
