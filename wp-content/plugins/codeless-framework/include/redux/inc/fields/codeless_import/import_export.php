	...
	public function __construct(){
		...
		function save($file, $content){
			...
			if(!file_put_contents($dir.$file, $content)){
				$this->message = __("Can't write file", "codeless");
				return false;
			}
			return true;		
        ...
        function codeless_import_content($file, $attachments){
				...
				require_once( cl_file_path( 'APP_ROOT', '/include/redux/inc/fields/codeless_import/wordpress-importer.php') );
		...
		function codeless_export_content(){
			...
			if($this->save('content.xml', $output)){
				$this->message = __("Content exported successfully", "codeless");
				return 1;
			}else{
				return 2;
			}
		...
		public function codeless_import_options($file){
			...
			if($options){
				...
				$this->message .= '<br />'.__("Options imported successfully", "codeless");
				return 1;
			}else{
				$this->message .= '<br />'.__("Your backup doesn't contain options.txt file.", "codeless");
				return 2;
			}
		}
		...
		public function codeless_export_options(){
			...
			if($this->save("options.txt", $content) ){
				$this->message = __("Options exported successfully", "codeless");
				return 1;
			}else{
				return 2;
			}
		...
		function codeless_import_widgets($file){
			...
			if($options){
				...
				$this->message .= '<br />'.__("Widgets Imported !", "codeless");
				return 1;
			}else{
		...
		function codeless_export_widgets_sidebars(){
			...
			if($this->save("sidebar_widgets.txt", $encoded)){
				$this->message = __("Widgets exported successfully", "codeless");
				return 1;
			}else{
				return 2;
			}
		...
		public function codeless_export_menu(){
			...
			if($this->save("menu.txt", $output)){
				$this->message = __("Menu exported successfully", "codeless");
				return 1;
			}else{
				return 2;
			}
...
/* ------------------------------------------------ AJAX -----------------------------------------------------------------*/
        ...
        function ajax_codeless_import_content()
		{
			...
			echo codeless_complex_esc( $status.$codelessCore->message );
			die();
		}
